...
// and why Static delegate instead of the usual Dynamic Delegate? because the static delegate supports lambdas better
// and integrates to native c++ well, this does however remove the functionality of unreal reflection system, but we
// don't need that here, as the blueprint latent function will handle that
DECLARE_DELEGATE_ThreeParams(FOnChatCompletionResponse, const FString&, const FString&, bool);

// Blueprint async delegate
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FGenChatCompletionDelegate, const FString&, Response, const FString&,
											   Error, bool, Success);
...
class GENERATIVEAISUPPORT_API UGenOAIChat : public UCancellableAsyncAction
{
  ...
  private:
	...
	static void MakeRequest(const FGenChatSettings& ChatSettings,
