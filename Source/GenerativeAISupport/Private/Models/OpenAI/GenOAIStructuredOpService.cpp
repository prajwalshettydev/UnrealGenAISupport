...
void UGenOAIStructuredOpService::RequestStructuredOutput(const FGenOAIStructuredChatSettings& StructuredChatSettings,
														 const FOnSchemaResponse& OnComplete)
{
	MakeRequest(StructuredChatSettings,
...
UGenOAIStructuredOpService*
UGenOAIStructuredOpService::RequestOpenAIStructuredOutput(UObject* WorldContextObject,
														  const FGenOAIStructuredChatSettings& StructuredChatSettings)
{
...
void UGenOAIStructuredOpService::MakeRequest(
	const FGenOAIStructuredChatSettings& StructuredChatSettings,
	const TFunction<void(const FString&, const FString&, bool)>& ResponseCallback)
{
	...
	for (const FGenChatMessage& Message : StructuredChatSettings.ChatSettings.Messages)
	{
		...
		{
			// in api documentation, it is mentioned that the system message should be appended with "Generate Response
			// in JSON only." todo, we can move this outside this scope, but that doesnt guarantee the callee will
			// append the message
			JsonMessage->SetStringField(
				TEXT("content"), Message.Content + TEXT(" Generate Response in JSON only. Use proper JSON formatting "
														"and avoid introducing line breaks inside string values."));
		}
 ...
 */
void UGenOAIStructuredOpService::ProcessResponse(
	const FString& ResponseStr, const TFunction<void(const FString&, const FString&, bool)>& ResponseCallback)
{
