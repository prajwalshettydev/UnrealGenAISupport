												...
												const FString& SavePath)
{
	...
	UBlueprint* Blueprint = Cast<UBlueprint>(Factory->FactoryCreateNew(
		UBlueprint::StaticClass(), Package, *BlueprintName, RF_Public | RF_Standalone, nullptr, GWarn));
	if (!Blueprint)
	{
...
bool UGenBlueprintUtils::AddVariable(const FString& BlueprintPath, const FString& VariableName,
									 const FString& VariableType, const FString& DefaultValue, const FString& Category)
{
	// Load the blueprint asset
	UBlueprint* Blueprint = LoadBlueprintAsset(BlueprintPath);
	if (!Blueprint)
	...
	if (!DefaultValue.IsEmpty())
	{
		...
		if (Property)
		{
			...
			FBlueprintEditorUtils::PropertyValueFromString(
				Property, DefaultValueToUse,
				reinterpret_cast<uint8*>(Blueprint->GeneratedClass->GetDefaultObject(true)));
		}
	}
	...
	for (auto& Input : Inputs)
	{
		...
		if (InputObj.IsValid())
		{
			...
			if (ParamType.Equals(TEXT("boolean"), ESearchCase::IgnoreCase))
			{
				...
				EntryNode->CreateUserDefinedPin(*ParamName, PinType, EGPD_Output);
			}
		...
		for (auto& Output : Outputs)
		{
			...
			if (OutputObj.IsValid())
			{
				...
				if (ParamType.Equals(TEXT("boolean"), ESearchCase::IgnoreCase))
				{
					FEdGraphPinType PinType;
					PinType.PinCategory = K2Schema->PC_Boolean;
					ResultNode->CreateUserDefinedPin(*ParamName, PinType, EGPD_Input);
				}
				else if (ParamType.Equals(TEXT("int"), ESearchCase::IgnoreCase))
				{
					FEdGraphPinType PinType;
										 ...
										 const FString& TargetNodeGuid, const FString& TargetPinName)
{
	...
	if (!SourcePin || !TargetPin)
	{
		...
		FString ErrorMessage = FString::Printf(TEXT("Pin not found. Source: %s (%s), Target: %s (%s)"), *SourcePinName,
											   SourcePin ? TEXT("found") : TEXT("not found"), *TargetPinName,
		...
		auto AddPins =
			[](UK2Node* Node, const FString& FieldName, TSharedPtr<FJsonObject> JsonObj, EEdGraphPinDirection Direction)
		{
	...
	else
	{
		...
		TSharedPtr<FJsonObject> ResponseObject = MakeShareable(new FJsonObject);
		ResponseObject->SetBoolField(TEXT("success"), false);
		ResponseObject->SetStringField(TEXT("error"),
									   TEXT("Failed to connect pins - type mismatch or invalid connection"));
				...
				if (Pin->Direction == EGPD_Output && Pin->PinType.PinCategory == UEdGraphSchema_K2::PC_Exec)
				{
					if (Pin->LinkedTo.Num() > 1)
					{
						...
						FString Message = FString::Printf(
							TEXT("Fixed invalid multiple connections from pin %s on node %s"), *Pin->PinName.ToString(),
											 ...
											 const FString& ConnectionsJson)
{
	...
	for (int32 i = 0; i < ConnectionsArray.Num(); i++)
	{
		...
		FString ConnectionResult =
			ConnectNodes(BlueprintPath, FunctionGuid, SourceNodeGuid, SourcePinName, TargetNodeGuid, TargetPinName);
	...
	UE_LOG(LogTemp, Log, TEXT("Connected %d/%d node pairs in blueprint %s"), SuccessfulConnections,
		   ConnectionsArray.Num(), *BlueprintPath);
...
bool UGenBlueprintUtils::OpenBlueprintGraph(UBlueprint* Blueprint, UEdGraph* Graph)
{
	...
	if (BlueprintEditor && Graph != nullptr)
	{
		...
		if (Graph != nullptr)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("Skipping opening empty graph in blueprint %s"), *Blueprint->GetName());
			}
		}
...
FString UGenBlueprintUtils::GetNodeGUID(const FString& BlueprintPath, const FString& GraphType, const FString& NodeName,
										const FString& FunctionGuid)
{
	...
	{
		if (NodeName.IsEmpty())
			return TEXT("{\"success\": false, \"error\": \"Node name required for EventGraph\"}");
		if (Blueprint->UbergraphPages.Num() > 0)
	...
	else if (GraphType.Equals(TEXT("FunctionGraph"), ESearchCase::IgnoreCase))
	{
		if (FunctionGuid.IsEmpty())
			return TEXT("{\"success\": false, \"error\": \"Function GUID required for FunctionGraph\"}");
		FGuid GraphGuid;
		if (!FGuid::Parse(FunctionGuid, GraphGuid))
			return TEXT("{\"success\": false, \"error\": \"Invalid function GUID\"}");
		for (UEdGraph* Graph : Blueprint->FunctionGraphs)
	...
	if (!TargetGraph)
		return TEXT("{\"success\": false, \"error\": \"Could not find specified graph\"}");
		...
		if (!TargetNode && (NodeName.Equals(TEXT("BeginPlay")) || NodeName.Equals(TEXT("Tick"))))
		{
			...
			if (Function)
			{
				...
				UK2Node_Event* NewEventNode = FKismetEditorUtilities::AddDefaultEventNode(
					Blueprint, TargetGraph, FName(*NodeName), BlueprintClass, NodePosY);
...
FString UGenBlueprintUtils::AddComponentWithEvents(const FString& BlueprintPath, const FString& ComponentName,
												   const FString& ComponentClassName)
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Could not load blueprint at path: %s"), *BlueprintPath);
		return TEXT("{\"success\": false, \"error\": \"Could not load blueprint\", \"begin_overlap_guid\": \"\", "
					"\"end_overlap_guid\": \"\"}");
	}
	...
	if (!SCS)
	{
		...
		return TEXT("{\"success\": false, \"error\": \"No SCS found\", \"begin_overlap_guid\": \"\", "
					"\"end_overlap_guid\": \"\"}");
	}
	...
	if (!ComponentClass || !ComponentClass->IsChildOf(UShapeComponent::StaticClass()))
	{
		UE_LOG(LogTemp, Error,
			   TEXT("Component class %s must be a collision component (e.g., BoxComponent, SphereComponent)"),
			   *ComponentClassName);
		return TEXT("{\"success\": false, \"error\": \"Not a collision component class\", \"begin_overlap_guid\": "
					"\"\", \"end_overlap_guid\": \"\"}");
	}
	...
	for (USCS_Node* Node : SCS->GetAllNodes())
	{
		if (Node->GetVariableName().ToString().Equals(ComponentName, ESearchCase::IgnoreCase))
		{
			...
			return TEXT("{\"success\": false, \"error\": \"Component already exists\", \"begin_overlap_guid\": \"\", "
						"\"end_overlap_guid\": \"\"}");
		}
	}
	...
	if (!NewComponentNode)
	{
		...
		return TEXT("{\"success\": false, \"error\": \"Failed to create component\", \"begin_overlap_guid\": \"\", "
					"\"end_overlap_guid\": \"\"}");
	}
	...
	if (!ShapeComp)
	{
		...
		return TEXT("{\"success\": false, \"error\": \"Invalid shape component template\", \"begin_overlap_guid\": "
					"\"\", \"end_overlap_guid\": \"\"}");
	}
	...
	if (!BeginOverlapDelegate)
	{
		...
		return TEXT("{\"success\": false, \"error\": \"No BeginOverlap delegate\", \"begin_overlap_guid\": \"\", "
					"\"end_overlap_guid\": \"\"}");
	}
	...
	UE_LOG(LogTemp, Log, TEXT("Spawned OnComponentBeginOverlap for %s with GUID %s"), *ComponentName,
		   *BeginOverlapEvent->NodeGuid.ToString());
	...
	if (!EndOverlapDelegate)
	{
		...
		return TEXT("{\"success\": false, \"error\": \"No EndOverlap delegate\", \"begin_overlap_guid\": \"\", "
					"\"end_overlap_guid\": \"\"}");
	}
	...
	UE_LOG(LogTemp, Log, TEXT("Spawned OnComponentEndOverlap for %s with GUID %s"), *ComponentName,
		   *EndOverlapEvent->NodeGuid.ToString());
	...
	return FString::Printf(TEXT("{\"success\": true, \"message\": \"Added collision component %s with overlap "
								"events\", \"begin_overlap_guid\": \"%s\", \"end_overlap_guid\": \"%s\"}"),
