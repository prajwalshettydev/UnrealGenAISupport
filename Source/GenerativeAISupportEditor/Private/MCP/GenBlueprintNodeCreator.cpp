                                                      ...
                                                      const FString& PropertiesJson)
{
	...
	UClass* NodeClass = FindFirstObjectSafe<UClass>(*(TEXT("UK2Node_") + ActualNodeType));
	if (!NodeClass || !NodeClass->IsChildOf(UK2Node::StaticClass()))
		NodeClass = FindFirstObjectSafe<UClass>(*ActualNodeType);
	if (NodeClass && NodeClass->IsChildOf(UK2Node::StaticClass()))
	{
                                                             ...
                                                             TArray<FString>& OutSuggestions)
{
	...
	{
		UClass* LibClass = FindFirstObjectSafe<UClass>(*LibraryName);
