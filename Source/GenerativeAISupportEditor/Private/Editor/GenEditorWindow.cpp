...
{
	TSharedRef<SDockTab> Tab = SNew(SDockTab).TabRole(ETabRole::NomadTab)[SNew(SGenEditorWindow)];
...
void SGenEditorWindow::Construct(const FArguments& InArgs)
{
	...
	ChildSlot[SNew(SVerticalBox)
			  ...
			  + SVerticalBox::Slot().FillHeight(
					1.0f)[SNew(SScrollBox)
						  // MCP Status Section
						  + SScrollBox::Slot().Padding(10)[SNew(SBox).WidthOverride(800) // Increased width to
						  ...
						  + SScrollBox::Slot().Padding(
								10)[SNew(SBox).WidthOverride(800) // Increased width to accommodate more columns
										[CreateActionButtonsSection()]]]
			  // Bottom buttons
			  +
			  SVerticalBox::Slot().AutoHeight().Padding(10).HAlign(
				  HAlign_Right)[SNew(SHorizontalBox) +
											  ...
											  .OnClicked_Lambda(
												  []()
												  {
													  FModuleManager::LoadModuleChecked<ISettingsModule>("Settings")
														  .ShowViewer("Project", "Plugins", "GenerativeAISupport");
													  return FReply::Handled();
												  })]]];
	...
	if (GEditor && GEditor->GetEditorWorldContext().World())
	{
		GEditor->GetEditorWorldContext().World()->GetTimerManager().SetTimer(
			StatusRefreshTimerHandle, FTimerDelegate::CreateSP(this, &SGenEditorWindow::RefreshStatus),
			   ...
			   + SVerticalBox::Slot().AutoHeight().Padding(0, 0, 0, 5)
					 [SNew(SHorizontalBox) +
					  ...
					  SHorizontalBox::Slot()
						  .AutoWidth()
						  .VAlign(VAlign_Center)
						  .Padding(0, 0, 10, 0)[SNew(STextBlock)
					  ...
					  SHorizontalBox::Slot().AutoWidth().VAlign(
						  VAlign_Center)[SNew(STextBlock)
											 ...
											 .Font(FCoreStyle::GetDefaultFontStyle("Bold", 11))]]
			   // Separator line
			   + SVerticalBox::Slot().AutoHeight().Padding(0, 0, 0, 10)[SNew(SBox).HeightOverride(
					 1.0f)[SNew(SBorder).BorderImage(FEditorStyle::GetBrush("Menu.Separator")).Padding(FMargin(0.0f))]]
			   ...
			   + SVerticalBox::Slot().AutoHeight().Padding(
					 0, 2)[SNew(SHorizontalBox) +
						   SHorizontalBox::Slot()
							   ...
							   .VAlign(VAlign_Center)
							   .Padding(0, 0, 10, 0)[SNew(STextBlock)
														 .Text(NSLOCTEXT("GenerativeAISupport", "UnrealSocketLabel",
						   ...
						   SHorizontalBox::Slot()
							   ...
							   .VAlign(VAlign_Center)
							   .Padding(0, 0, 10,
										0)[SAssignNew(UnrealSocketStatusText, STextBlock)
											   .Text(NSLOCTEXT("GenerativeAISupport", "CheckingStatus", "Checking..."))
											   .MinDesiredWidth(100)] +
						   SHorizontalBox::Slot()
							   .FillWidth(1.0f)
							   .VAlign(VAlign_Center)
							   .Padding(0, 0, 10, 0)[SAssignNew(UnrealSocketDetailsText, STextBlock)
														 .Text(NSLOCTEXT("GenerativeAISupport", "NoDetails", ""))
														 .MinDesiredWidth(150)] +
						   SHorizontalBox::Slot()
							   .AutoWidth()
							   .VAlign(VAlign_Center)
							   .HAlign(HAlign_Right)[SNew(SBox).WidthOverride(100).Visibility_Lambda(
								   [this]() -> EVisibility
								   {
									   return IsUnrealSocketServerRunning() ? EVisibility::Collapsed
																			: EVisibility::Visible;
								   })[SNew(SButton)
										  ...
										  .OnClicked_Lambda(
											  [this]()
											  {
												  ...
												  UGenerativeAISupportSettings* Settings =
												  ...
												  if (Settings)
												  {
													  ...
													  FNotificationInfo Info(
														  NSLOCTEXT("GenerativeAISupport", "SocketServerStarted",
																	"Socket server auto-start enabled. The server will "
										  ...
										  .ToolTipText(NSLOCTEXT("GenerativeAISupport", "StartSocketTooltip",
																 "Enable auto-start for the Unreal Socket Server"))]]]
			   // MCP Server Status
			   + SVerticalBox::Slot().AutoHeight().Padding(
					 0, 2)[SNew(SHorizontalBox) +
						   SHorizontalBox::Slot()
						   ...
						   SHorizontalBox::Slot()
							   ...
							   .VAlign(VAlign_Center)
							   .Padding(0, 0, 10,
										0)[SAssignNew(MCPServerStatusText, STextBlock)
											   .Text(NSLOCTEXT("GenerativeAISupport", "CheckingStatus", "Checking..."))
											   .MinDesiredWidth(100)] +
						   SHorizontalBox::Slot()
							   .FillWidth(1.0f)
							   .VAlign(VAlign_Center)
							   .Padding(0, 0, 10, 0)[SAssignNew(MCPServerDetailsText, STextBlock)
														 .Text(NSLOCTEXT("GenerativeAISupport", "NoDetails", ""))
														 .MinDesiredWidth(150)] +
						   SHorizontalBox::Slot()
							   .AutoWidth()
								   ...
								   [this]() -> EVisibility
								   {
									   return IsMCPServerRunning() ? EVisibility::Collapsed : EVisibility::Visible;
										  ...
										  .OnClicked_Lambda(
											  [this]()
											  {
												  const FText Title = NSLOCTEXT(
													  "GenerativeAISupport", "MCPServerSetupTitle", "Setup MCP Server");
												  const FText Message =
													  NSLOCTEXT("GenerativeAISupport", "MCPServerSetupMessage",
																"To use MCP Server with AI tools, you need to set up "
																"at least one of the following:\n\n"
																"- Set up Claude configuration\n"
																"- Set up Cursor configuration\n\n"
																"After setup, you'll need to restart Claude or Cursor "
																"to activate the MCP Server.");

												  FMessageDialog::Open(EAppMsgType::Ok, Message, &Title);
												  return FReply::Handled();
											  })
										  .ToolTipText(NSLOCTEXT("GenerativeAISupport", "SetupMCPTooltip",
			   ...
			   SVerticalBox::Slot().AutoHeight().Padding(0, 2)
				   [SNew(SHorizontalBox) +
					...
					SHorizontalBox::Slot()
						...
						.VAlign(VAlign_Center)
						.Padding(0, 0, 10,
								 0)[SAssignNew(ClaudeStatusText, STextBlock)
										.Text(NSLOCTEXT("GenerativeAISupport", "CheckingStatus", "Checking..."))
										.MinDesiredWidth(100)] +
					SHorizontalBox::Slot()
						.FillWidth(1.0f)
						.VAlign(VAlign_Center)
						.Padding(0, 0, 10, 0)[SAssignNew(ClaudeDetailsText, STextBlock)
												  .Text(NSLOCTEXT("GenerativeAISupport", "NoDetails", ""))
												  .MinDesiredWidth(150)
												  .ToolTipText_Lambda(
													  [this]() { return FText::FromString(GetClaudeConfigPath()); })] +
					SHorizontalBox::Slot()
						.AutoWidth()
						.VAlign(VAlign_Center)
						.HAlign(HAlign_Right)[SNew(SButton)
												  ...
												  .IsEnabled_Lambda(
													  [this]() { return FPaths::FileExists(GetClaudeConfigPath()); })]]
			   // Cursor Status
			   +
			   SVerticalBox::Slot().AutoHeight().Padding(0, 2)
				   [SNew(SHorizontalBox) +
					...
					SHorizontalBox::Slot()
						...
						.VAlign(VAlign_Center)
						.Padding(0, 0, 10,
								 0)[SAssignNew(CursorStatusText, STextBlock)
										.Text(NSLOCTEXT("GenerativeAISupport", "CheckingStatus", "Checking..."))
										.MinDesiredWidth(100)] +
					SHorizontalBox::Slot()
						.FillWidth(1.0f)
						.VAlign(VAlign_Center)
						.Padding(0, 0, 10, 0)[SAssignNew(CursorDetailsText, STextBlock)
												  .Text(NSLOCTEXT("GenerativeAISupport", "NoDetails", ""))
												  .MinDesiredWidth(150)
												  .ToolTipText_Lambda(
													  [this]() { return FText::FromString(GetCursorConfigPath()); })] +
					SHorizontalBox::Slot()
						.AutoWidth()
						.VAlign(VAlign_Center)
						.HAlign(HAlign_Right)
							[SNew(SButton)
								 ...
								 .IsEnabled_Lambda([this]() { return FPaths::FileExists(GetCursorConfigPath()); })]]];
}
	...
	return SNew(SBorder)
		...
		.Padding(
			8)[SNew(SVerticalBox)
			   ...
			   + SVerticalBox::Slot().AutoHeight()[SAssignNew(APIStatusContainer, SVerticalBox)]];
}
		...
		.Padding(8)[SNew(SVerticalBox)
						  ...
						  0, 4)[SNew(SHorizontalBox) +
								SHorizontalBox::Slot().FillWidth(
									1.0f)[SNew(STextBlock)
											  .Text(NSLOCTEXT("GenerativeAISupport", "ClaudeConfigLabel",
															  "Create/Update Claude Configuration"))
											  .ToolTipText(NSLOCTEXT("GenerativeAISupport", "ClaudeConfigTooltip",
																	 "Creates or updates Claude's configuration file "
					...
					+ SVerticalBox::Slot().AutoHeight().Padding(
						  0, 4)[SNew(SHorizontalBox) +
								SHorizontalBox::Slot().FillWidth(
									1.0f)[SNew(STextBlock)
											  .Text(NSLOCTEXT("GenerativeAISupport", "CursorConfigLabel",
															  "Create/Update Cursor Configuration"))
											  .ToolTipText(NSLOCTEXT("GenerativeAISupport", "CursorConfigTooltip",
																	 "Creates or updates Cursor's MCP configuration "
																	 "file to work with this plugin"))] +
								SHorizontalBox::Slot().AutoWidth().Padding(
									10, 0, 0,
									0)[SNew(SButton)
...
void SGenEditorWindow::RefreshStatus()
{
	...
	UnrealSocketStatusText->SetText(bUnrealSocketRunning
										? NSLOCTEXT("GenerativeAISupport", "StatusRunning", "Running ✓")
										: NSLOCTEXT("GenerativeAISupport", "StatusNotRunning", "Not Running ❌"));
	UnrealSocketStatusText->SetColorAndOpacity(bUnrealSocketRunning ? FLinearColor(0.0f, 0.8f, 0.0f)
																	: NotConfiguredColor);
	...
	MCPServerStatusText->SetText(bMCPServerRunning ? NSLOCTEXT("GenerativeAISupport", "StatusRunning", "Running ✓")
												   : NSLOCTEXT("GenerativeAISupport", "StatusNotRunning", ""));
	MCPServerStatusText->SetColorAndOpacity(bMCPServerRunning ? FLinearColor(0.0f, 0.8f, 0.0f) : NotConfiguredColor);
	...
	ClaudeStatusText->SetText(bClaudeConfigured
	...
	ClaudeStatusText->SetColorAndOpacity(bClaudeConfigured ? FLinearColor(0.0f, 0.8f, 0.0f) : NotConfiguredColor);
	...
	CursorStatusText->SetText(bCursorConfigured
								  ? NSLOCTEXT("GenerativeAISupport", "StatusConfigured", "Configured ✓")
								  : NSLOCTEXT("GenerativeAISupport", "StatusNotConfigured", "Not Configured ❌"));
	CursorStatusText->SetColorAndOpacity(bCursorConfigured ? FLinearColor(0.0f, 0.8f, 0.0f) : NotConfiguredColor);
	...
	for (const auto& OrgPair : OrgDisplayNames)
	{
		...
		APIStatusContainer->AddSlot().AutoHeight().Padding(
			0,
			2)[SNew(SHorizontalBox) +
			   SHorizontalBox::Slot()
				   .AutoWidth()
				   .VAlign(VAlign_Center)
				   .Padding(0, 0, 10, 0)[SNew(STextBlock).Text(FText::FromString(OrgName)).MinDesiredWidth(150)] +
			   SHorizontalBox::Slot()
				   .AutoWidth()
				   .VAlign(VAlign_Center)
				   .Padding(0, 0, 10,
							0)[SNew(STextBlock)
								   .Text(bKeySet ? NSLOCTEXT("GenerativeAISupport", "StatusConfigured", "Configured ✓")
												 : NSLOCTEXT("GenerativeAISupport", "StatusNotConfigured",
															 "Not Configured ❌"))
								   .ColorAndOpacity(bKeySet ? FLinearColor(0.0f, 0.8f, 0.0f) : NotConfiguredColor)
								   .MinDesiredWidth(100)] +
...
bool SGenEditorWindow::IsMCPServerRunning() const
{
	...
	FString PidFilePath =
		FPaths::Combine(FPlatformProcess::UserHomeDir(), TEXT(".unrealgenai"), TEXT("mcp_server.pid"));
			...
			if (Lines.Num() >= 1)
			{
				...
				if (ProcessId > 0 && FPlatformProcess::IsApplicationRunning(ProcessId))
				{
					...
					if (Lines.Num() >= 2)
					{
						...
						MCPServerDetailsText->SetText(
							FText::FromString(FString::Printf(TEXT("Running on port %s"), *Lines[1])));
					}
					return true;
				}
			}
	...
	return (IsClaudeConfigured() || IsCursorConfigured()) && FPlatformProcess::IsApplicationRunning(TEXT("python"));
}
...
void SGenEditorWindow::OpenClaudeConfig() const
{
	...
	else
	{
		...
		FMessageDialog::Open(EAppMsgType::Ok,
							 FText::Format(NSLOCTEXT("GenerativeAISupport", "FileNotFoundDetail",
													 "Claude config file not found:\n{0}\n\nWe'll create it in this "
													 "location when you click 'Setup Claude'"),
...
FReply SGenEditorWindow::SetupClaudeConfig()
{
	...
	{
		FMessageDialog::Open(EAppMsgType::Ok, NSLOCTEXT("GenerativeAISupport", "PluginNotFound",
														"GenerativeAISupport plugin not found!"));
		return FReply::Handled();
	}

	FString ConfigJson = FString::Printf(
	...
	if (bSuccess)
	{
		FNotificationInfo Info(FText::Format(NSLOCTEXT("GenerativeAISupport", "ClaudeConfigSuccessDetail",
													   "Claude configuration updated successfully at:\n{0}"),
											 FText::FromString(ClaudeConfigPath)));
	...
	{
		FNotificationInfo Info(
			FText::Format(NSLOCTEXT("GenerativeAISupport", "ClaudeConfigFailedDetail",
									"Failed to update Claude configuration at:\n{0}\nPlease check folder permissions."),
						  FText::FromString(ClaudeConfigPath)));
		Info.ExpireDuration = 7.0f;
		FSlateNotificationManager::Get().AddNotification(Info);
	}
	...
	else
	{
		...
		FMessageDialog::Open(EAppMsgType::Ok,
							 FText::Format(NSLOCTEXT("GenerativeAISupport", "FileNotFoundDetail",
													 ...
													 "location when you click 'Setup Cursor'"),
...
FReply SGenEditorWindow::SetupCursorConfig()
{
	...
	if (bSuccess)
	{
		FNotificationInfo Info(FText::Format(NSLOCTEXT("GenerativeAISupport", "CursorConfigSuccessDetail",
													   "Cursor configuration updated successfully at:\n{0}"),
											 FText::FromString(CursorConfigPath)));
		Info.ExpireDuration = 7.0f;
		FSlateNotificationManager::Get().AddNotification(Info);
		OpenCursorConfig();
	...
	{
		FNotificationInfo Info(
			FText::Format(NSLOCTEXT("GenerativeAISupport", "CursorConfigFailedDetail",
									"Failed to update Cursor configuration at:\n{0}\nPlease check folder permissions."),
						  FText::FromString(CursorConfigPath)));
		Info.ExpireDuration = 7.0f;
		FSlateNotificationManager::Get().AddNotification(Info);
	}
